...
export default function Home() {
  ...
  const spursUpcoming = useMemo(() => live?.matches.filter(match => (match.status === "SCHEDULED" || match.status === "TIMED") && (match.home.includes("Tottenham") || match.away.includes("Tottenham"))).slice(0, 6) ?? [], [live]);
  const scenarioOverrides = useMemo<ScenarioOverride[]>(() => spursUpcoming.flatMap(match => { const result = scenario[`${match.home}-${match.away}`]; return result && result !== "AUTO" ? [{ home: normalizeTeamName(match.home), away: normalizeTeamName(match.away), result }] : []; }), [scenario, spursUpcoming]);
  ...
  return <main className="shell">
    ...
    <div className="grid">
      ...
      <section className="panel full"><h2>What-if scenario engine</h2>{spursUpcoming.length ? <><p className="notice">Override upcoming Spurs results and watch the season simulation recalculate.</p><div className="scenario-grid">{spursUpcoming.map(match => { const key = `${match.home}-${match.away}`; return <div className="scenario-row" key={key}><span>{normalizeTeamName(match.home)} v {normalizeTeamName(match.away)}</span><select value={scenario[key] ?? "AUTO"} onChange={event => setScenario(current => ({...current, [key]: event.target.value as "AUTO" | "WIN" | "DRAW" | "LOSS"}))}><option value="AUTO">Model prediction</option><option value="WIN">Spurs win</option><option value="DRAW">Draw</option><option value="LOSS">Spurs lose</option></select></div>; })}</div></> : <p className="notice">Connect live data to configure upcoming Spurs scenarios.</p>}</section>
