...
import { eloRatings, normalizeTeamName, prediction, simulation, teams, type Match } from "../lib/analytics";
...
export default function Home() {
  ...
  const liveResults = useMemo<Match[] | undefined>(() => {
    const completed = live?.matches.filter(match => match.homeGoals !== null && match.awayGoals !== null) ?? [];
  ...
  return <main className="shell">
    ...
    <div className="grid">
      ...
      <section className="panel full"><h2>Where will Spurs finish? <span style={{color:"var(--muted)",fontWeight:400}}>5,000 season simulations</span></h2><table className="table"><thead><tr><th>Team</th><th>Expected finish</th><th>Top 4</th><th>Top 6</th></tr></thead><tbody>{table.map(row=><tr key={row.team}><td className={row.team==="Tottenham"?"highlight":""}>{row.team}</td><td>{row.avg.toFixed(1)}</td><td>{pct(row.top4)}</td><td>{pct(row.top6)}</td></tr>)}</tbody></table><p className="notice">The match predictor and Elo ratings use completed live results when available. The season simulator remains on demo fixtures until the full current-season schedule is connected.</p></section>
